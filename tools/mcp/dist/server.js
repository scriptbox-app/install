...
import { ConfirmationStore, PathPolicy, redact } from './core.js';
...
function buildServer() {
    ...
    for (const project of ['install', 'ui', 'api'])
        resource(server, `${project}-status`, `scriptbox://projects/${project}/status`, () => projectStatus(project));
    ...
    server.registerTool('package_validate', { title: 'Validate a package ZIP', description: 'Accepts only files under configured artifact input roots.', annotations: { readOnlyHint: true }, inputSchema: z.object({ path: z.string() }) }, async ({ path: input }) => {
        const file = await policy.resolveArtifact(input);
        const command = commandFor('api', 'package_validate');
...
function resource(server, name, uri, callback) {
...
async function projectStatus(project) {
    const root = policy.projectRoot(project);
    const branch = await git(root, ['branch', '--show-current']);
    const changed = await git(root, ['status', '--porcelain']);
...
async function runNamed(project, operation) { return runFixed(commandFor(project, operation), policy.projectRoot(project)); }
async function checkProject(project) {
    const operations = project === 'install' ? ['test', 'build', 'syntax'] : project === 'ui' ? ['test', 'lint', 'build'] : ['test', 'syntax'];
    const results = [];
    for (const operation of operations)
        results.push({ operation, ...(await runNamed(project, operation)) });
    return { project, ok: results.every((result) => result.ok), results };
}
async function validateWorkspace() { const projects = []; for (const project of ['install', 'ui', 'api'])
    projects.push(await checkProject(project)); return { ok: projects.every((item) => item.ok), projects }; }
async function currentReleases() {
    const installer = await hashPath(path.join(policy.projectRoot('install'), 'install.php'));
    let ui = null;
    try {
        ui = await hashPath(await realpath(path.join(policy.projectRoot('ui'), 'dist')));
    }
    catch { }
...
async function publishPreview(input) {
    ...
    const operation = { ...input, path: target, sha256: await hashPath(target) };
    const confirmation = await confirmations.create(operation);
    await confirmations.audit({ action: 'publish_preview', operation });
...
async function publishExecute(token) {
    ...
    if (target !== operation.path || await hashPath(target) !== operation.sha256)
        throw new Error('Publication input changed after preview');
...
async function hashPath(input) {
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
        if (entry.isSymbolicLink())
            throw new Error('Symlinks are forbidden in publish inputs');
        hash.update(entry.name);
        hash.update(await hashPath(path.join(input, entry.name)));
    }
    return hash.digest('hex');
}
function toolResult(value, redactSecrets = true) { const clean = redactSecrets ? redact(value) : value; return { content: [{ type: 'text', text: JSON.stringify(clean, null, 2) }], structuredContent: clean }; }
