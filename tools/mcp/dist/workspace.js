...
export function commandFor(project, operation) {
    const value = COMMANDS[project]?.[operation];
    if (!value)
        throw new Error(`Unsupported fixed command: ${project}/${operation}`);
...
export async function runFixed(command, cwd, extraArgs = [], env = { PATH: process.env.PATH }) {
    return new Promise((resolve, reject) => {
        ...
        let output = '';
        const capture = (chunk) => { if (output.length < 128 * 1024)
...
function sanitizeOutput(output) {
    const scrubbed = output
        ...
        .replace(/[\t ]+$/gm, '');
    const safeLines = scrubbed.split('\n').filter((line) => /^(?:ok|not ok|# Subtest:|TAP version|1\.\.|tests?|suites?|pass|fail|cancelled|skipped|todo|duration_ms|No syntax errors|✓|dist\/|vite v|found 0 vulnerabilities|npm notice run|\s*\d+ (?:tests|problems)|\s*[A-Za-z0-9_./-]+\.(?:js|jsx|ts|tsx|php)\s*$)/i.test(line.trim()));
