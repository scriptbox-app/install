...
Executable SQL may not read or write server files or load native code, even when nested inside an otherwise allowlisted statement. Rejected constructs include MySQL `LOAD_FILE`, `INTO OUTFILE`/`DUMPFILE`, and plugin/component installation; PostgreSQL `pg_read_file`, `pg_read_binary_file`, `lo_import`, `lo_export`, `pg_ls_dir` and the documented `pg_ls_*dir` variants, `pg_stat_file`, and `pg_file_write`/`sync`/`rename`/`unlink`; SQLite `readfile`, `writefile`, and `load_extension`; and SQL Server `OPENROWSET`, `OPENDATASOURCE`, `OPENQUERY`, `BULK INSERT`, `xp_cmdshell`, and `CREATE ASSEMBLY`/`ALTER ASSEMBLY`. Detection uses the declared database dialect, including MySQL/MariaDB comment rules, executable `/*!...*/` and `/*M!...*/` comments, PostgreSQL array brackets and tagged or untagged dollar-quoted strings, and SQL Server bracket identifiers. Genuine quoted literals and inert comments remain data; unterminated PostgreSQL dollar strings are rejected.

Migration SQL must use SQL-standard doubled quotes (`'O''Reilly'`) rather than backslash-quote escapes. A sequence such as `\'` or `\"` is rejected for MySQL, MariaDB, and PostgreSQL because its meaning can change with `NO_BACKSLASH_ESCAPES` or the PostgreSQL string form. Put customer values and paths in parameter descriptors instead. Backslashes inside bound scalar values are data and remain supported.
